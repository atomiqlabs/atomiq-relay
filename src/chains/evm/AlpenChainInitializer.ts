...
import {
    EVMSpvVaultData,
    EVMFees, EVMSwapData, JsonRpcProviderWithRetries, WebSocketProviderWithRetries, AlpenChainType, initializeAlpen
...
export const AlpenChainInitializer: ChainInitializer<AlpenChainType, any, typeof template> = {
    loadChain: (directory, configuration, bitcoinRpc, bitcoinNetwork) => {
        ...
        const {chainInterface, btcRelay, swapContract, spvVaultContract} = initializeAlpen({
            rpcUrl: provider,
